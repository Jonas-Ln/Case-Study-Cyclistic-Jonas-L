...
So to Expand on the above we reverse the task and get the Following result:
...
The **next Step** is to evaluate what role riding time plays in the context of casual_riders and how it differs from members.
...
The Mean for each station suggests that the members ride on Average **32.45%** faster than casual riders, which suggests that the starting station has no significant influence on the Speed. But the Linear model suggests that the distance_minute is not significant when we add it to the linear model from chapter 3 which is why the linear model does not explain more with adding the variable of **mean_distance_time** (R^2 = 66.11%).
That is what the p value 0.652 > 0.05 suggests.
...
+ We plot the whole dataset in Tableau and get the Following distribution as a results:
